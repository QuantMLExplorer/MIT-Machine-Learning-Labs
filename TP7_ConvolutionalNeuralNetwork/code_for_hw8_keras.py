...
def run_keras_2d(data_name, layers, epochs, display=True, split=0.25, verbose=True, trials=1):
    ...
    y_train = to_categorical(y, num_classes)  # one-hot encoding
    y_val = to_categorical(y2, num_classes) if X_val is not None else None
    y_test = to_categorical(y3, num_classes) if X_test is not None else None
    
    val_acc, test_acc = 0, 0
    for trial in range(trials):
        # Create a new model instance in each trial to reset weights
        model = Sequential(layers)
        
        # Compile and run the model
        model, history, vacc, tacc = run_keras(
            X_train, y_train, X_val, y_val, X_test, y_test, layers, epochs,
            split=split, verbose=verbose
        )
        
        # Accumulate accuracy scores for averaging
    ...
    # Print average accuracy if applicable
    if val_acc:
